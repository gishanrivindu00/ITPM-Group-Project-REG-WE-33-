...
filterData(sales,searchKey){
  ...
  post.Sales_ID.toLowerCase().includes(searchKey)||
  ...
  post.Total_Amount.toLowerCase().includes(searchKey)||
  post.Sale_Date.toLowerCase().includes(searchKey)
  )
          ...
          <tbody>
            {this.state.sales.map((sales,index) =>(
              <tr key={index}>
                ...
                <td>
                    <a href={`/sales/sales/${sales._id}`} style={{textDecoration:'none'}}>
                    {sales.Sales_ID}
                    </a>
                </td>
