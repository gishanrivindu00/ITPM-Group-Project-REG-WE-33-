    ...
    return (
      <div className="container1">
        ...
        <table className="table table-success table-striped" style={{marginTop:'40px'}}>
          ...
          <tbody>
            {this.state.agents.map((agents,index) =>(
              <tr key={index}>
                <th scope="row">{index+1}</th>
                <td>
                    <a href={`/agents/${agents._id}`} style={{textDecoration:'none'}}>
                    {agents.Agent_ID}
                    </a>
                </td>
                <td>{agents.Agent_Name}</td>
                <td>{agents.District}</td>
                <td>{agents.Telephone_No}</td>
                <td>
                  <a className="btn btn-warning" href={`/agents/edit/${agents._id}`}>
                    <i className="fas fa-edit"></i>&nbsp;Edit
                  </a>
                  &nbsp;
                  <a className="btn btn-danger" href="#" onClick={() =>this.onDelete(agents._id)}>
